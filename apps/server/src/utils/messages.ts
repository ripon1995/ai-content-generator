...
export const MONGO_DB_MESSAGES = {
  ...
  CONNECTION_FAILED: (error: any): string =>
    `MongoDB connection failed. Error details: ${error.message || String(error)}`,
  MONGO_DISCONNECTED: 'MongoDB disconnected',
  MONGO_DB_ERROR: (error: any): string =>
    `MongoDB error. Error details: ${error.message || String(error)}`,
...
export const REFRESH_TOKEN_MESSAGES: TMessages = {
